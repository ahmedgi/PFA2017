app
  .controller('SettingsCtrl', function ($scope, $http, $filter, settingFactory, multipartForm) {
    ...
    $scope.ajouteruniv = function () {
      ...
      var univ = {
        "nom": $scope.univ.nom,
        "abrev": $scope.univ.abrev,
        "etablissements": $scope.addetab,
        "departements": $scope.adddep
      };
      $scope.adduniv = false;
    ...
    $scope.valider = function () {//add a new prof to the list of profs (items)
      ...
      var obj = {
        ...
        type: $scope.user.selectedtype,
        grade: $scope.user.selectedgrade
      };
    ...
    $scope.confirmPrivilege = function () {//when click on confirm(after privilege), change the privilege of a prof
      ...
      if ($scope.masks.prof) {
        $scope.activeObj.security_mask |= 1;
      }
      if ($scope.masks.filliere) {
        $scope.activeObj.security_mask |= 2;
      }
      if ($scope.masks.departement) {
        $scope.activeObj.security_mask |= 4;
      }
      if ($scope.masks.admin) {
        $scope.activeObj.security_mask |= 8;
      }
    ...
    $scope.matiere = function (obj) {
      ...
      $scope.test = $filter('filter')($scope.matieres, function (d) {
        return d.idProf == obj._id
      });
    };
  });
