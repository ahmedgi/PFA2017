...
app.service('profsList', function (profService, $rootScope, $window) {
  ...
  var getCurrentUser = function () {
    return profService.getCurrentUser().then(function successCallback(response) {
...
app.service('modulesList', function (moduleService, profsList, $rootScope, $filter) {
  ...
  var load = function () {
    items = [];
    return moduleService.load({
      userId: profsList.getUser()._id,
      searchQuery: {/*createdBy : profsList.getUser()._id ,*/'coordonnateur': {$in: [profsList.getUser()._id]}},
      responseFields: '',
      populate: [{path: 'createdBy', select: 'nom prenom'}, {
        path: 'updatedBy',
        select: 'nom prenom'
      }, {path: 'coordonnateur', select: 'nom prenom'}, {path: 'eModules', select: 'intitulee'}]
    })
      .then(function successCallback(response) {
...
app.service('moduleNotifList', function ($rootScope, moduleNotifService, profService, profsList) {
  ...
  var load = function () {
    return profService.getProfs({
      userId: profsList.getUser()._id,
      searchQuery: {_id: profsList.getUser()._id},
      responseFields: 'notification.moduleNotif'
    })
      .then(function successCallback(response) {
          return moduleNotifService.getNotif({
            userId: profsList.getUser()._id,
            searchQuery: {_id: {$in: response.data.data[0].notification.moduleNotif}},
            populate: [{path: 'module', select: 'intitulee'}, {path: 'eModule', select: 'intitulee'}, {
              path: 'prof',
              select: 'nom prenom'
            }]
          })
...
app.service('eModulesList', function (eModuleService, profsList, $rootScope, $filter) {
  ...
  var load = function () {
    items = [];
    return eModuleService.load({
      userId: profsList.getUser()._id,
      searchQuery: {/*createdBy : profsList.getUser()._id ,*/'sendTo._id': {$in: [profsList.getUser()._id]}},
      responseFields: '',
      populate: [{path: 'createdBy', select: 'nom prenom'}, {
        path: 'updatedBy',
        select: 'nom prenom'
      }, {path: 'sendTo._id', select: 'nom prenom'}]
    })
      .then(function successCallback(response) {
          items = items.concat(response.data.data);
          return eModuleService.load({
...
app.service('eModuleNotifList', function ($rootScope, eModuleNotifService, profService, profsList) {
  ...
  var load = function () {
    return profService.getProfs({
      userId: profsList.getUser()._id,
      searchQuery: {_id: profsList.getUser()._id},
      responseFields: 'notification.eModuleNotif'
    })
      .then(function successCallback(response) {
          return eModuleNotifService.getNotif({
            userId: profsList.getUser()._id,
            searchQuery: {_id: {$in: response.data.data[0].notification.eModuleNotif}},
            populate: [{path: 'eModule', select: 'intitulee'}, {path: 'prof', select: 'nom prenom'}]
          })
            .then(function successCallback(response) {
              count = 0;
              items = response.data.data;
  ...
  $scope.moduleTable = {
    ...
    menuOptions: [
      ...
      ['Télécharger', function ($itemScope) {
        moduleService.generatePDF({
          userId: $scope.user()._id,
          moduleId: modulesList.getItems()[modulesList.getSelectedItemIndex()]._id
        })
...
app.controller('gestionFilierController', function ($window, $scope, $rootScope, profService, modulesList, profsList, eModulesList, eModuleNotifList, moduleNotifList, moduleNotifService, eModuleNotifService, filiereList) {
  ...
  $rootScope.socket.on('newModuleNotif', function (notifId) {
    moduleNotifService.getNotif({
      userId: $scope.user()._id,
      searchQuery: {_id: notifId},
      populate: [{path: 'prof', select: 'nom prenom'}, {path: 'module', select: 'status'}, {
        path: 'eModule',
        select: 'intitulee status'
      }]
    })
      .then(function (response) {
  ...
  $rootScope.socket.on('newEmoduleNotif', function (notifId) {
    eModuleNotifService.getNotif({
      userId: $scope.user()._id,
      searchQuery: {_id: notifId},
      populate: [{path: 'prof', select: 'nom prenom'}, {path: 'eModule', select: 'status'}]
    })
      .then(function (response) {
        if (response.data.code == '200' && response.data.data.length > 0) {
          var notif = response.data.data[0];
